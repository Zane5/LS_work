...
class Participant
  STATUS = { ready: 'Ready', stay: 'Staying' }
...
class TwentyOne
  ...
  def player_turn
    ...
    loop do
      ...
      if answer == 's'
        player.status = Participant::STATUS[:stay]
  ...
  def dealer_should_stay?
    dealer.total >= 17 && !dealer.busted?
  end

  def dealer_turn
    puts "#{dealer.name}'s trun..."

    loop do
      if dealer_should_stay?
